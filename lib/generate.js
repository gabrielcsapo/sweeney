...
module.exports = function generate(directory, config) {
  ...
  posts.forEach((post) => {
    const out = render(directory, `layouts/${(post.template || 'post')}`, Object.assign({ post, page: {}, site: {} }, config));

    fs.writeFileSync(path.resolve(directory, 'site', `${post.file}.html`), out);
  });
  ...
  root.forEach((r) => {
    const out = render(directory, r.file, Object.assign({ posts, page: {}, site: {} }, config));

    fs.writeFileSync(path.resolve(directory, 'site', `${r.file}.html`), out);
  });
};
