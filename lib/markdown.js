  ...
  toHTML(ast) {
    ...
    ast.forEach((a) => {
      switch(a.type) {
        ...
        break;
        case 'inlineCode':
          html += `<code>${a.content}</code>`;
        break;
        case 'codeBlock':
          html += `<pre><code>${a.content}</code></pre>`;
        break;
        case 'table':
          html += `<table>
            ${a.header ? `<tr>
              ${a.header.map((h) => `<th>${this.toHTML(h)}</th>`).join('')}
            <tr>` : ''}
            ${a.cells ?
              a.cells.map((row) => {
                return `<tr>
                  ${row.map((c) => `<td>${this.toHTML(c)}</td>`).join('')}
                </tr>`;
              }).join('')
            : ''}
          </table>`;
    ...
    return createBodyParser('markdown',
      ...
      createParser('table', /^ *\|(.+)\n *\|( *[-:]+[-| :]*)\n((?: *\|.*(?:\n|$))*)\n*/, (src, pattern, match) => {
          ...
          function parseTableHeader(capture) {
              return capture[1].replace(TABLE_HEADER_TRIM, '').split(TABLE_ROW_SPLIT).map((c) => self.parse(c));
          }
          ...
          function parseTableCells(capture) {
              ...
              return rowsText.map((rowText) => {
                  return rowText.replace(PLAIN_TABLE_ROW_TRIM, '').split(TABLE_ROW_SPLIT).map((c) => self.parse(c));
              });
          }
      ...
      createParser('codeBlock', /^ *(`{3,}|~{3,})[ .]*(\S+)? *\n([\s\S]*?)\s*\1 *(?:\n+|$)/, (src, pattern, match) => {
        return [{ type: 'codeBlock', content: match[3] }, src.replace(pattern, '')];
      }),
