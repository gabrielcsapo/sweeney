...
const { ms, merge, escapeRegexValues, getConfig, templateToString } = require('../util')

function makeSearchable (object, _search, parent) {
  const search = _search || {}

  Object.keys(object).forEach((key) => {
    if (typeof object[key] === 'object' && !Array.isArray(object[key])) {
      makeSearchable(object[key], search, key)
    } else if (Array.isArray(object[key])) {

    } else {
      if (parent) {
        search[`${parent}.${key}`] = object[key]
      } else {
...
function getEditable (page) {
  ...
  if (page.data && page.data.editable) {
    const searchAbleObject = makeSearchable(page.data)
    ...
    page.data.editable.forEach((_editable) => {
      ...
      editable[_editable.filePath][_editable.variableName] = {
        type: _editable.type,
        value: searchAbleObject[_editable.variableName]
      }
    })
  }
        ...
        return form.parse(req, async (error, fields, files) => {
          ...
          Object.keys(fields).forEach(async (key) => {
            ...
            if (key.indexOf('options') === -1) {
              ...
              // TODO: we need to make sure if this is a number we don't add quotes
              rawConfig = rawConfig.replace(reg, function (value) {
                return `'${finalKey}':'${fields[key]}'`
              })
