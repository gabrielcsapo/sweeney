...
class Site {
  ...
  constructor(directory, config) {
    this.directory = config.source ? path.resolve(process.cwd(), config.source) : directory;
    this.config = config;
  ...
  async generate() {
    ...
    const posts = await Site.getTemplates(`${directory}/posts`, true, 'post');
    const root = await Site.getTemplates(directory, false, 'page');
    ...
    root.concat(posts).forEach(async function(post) {
      ...
      let options = Object.assign({ content: post.content, posts, page: {}, site: {} }, config);
      options[post.type] = post;
      
      await writeFile(`${output}/${post.file}.html`, await renderTemplate.bind(self)(directory, post.path, options, output));
    });
  }
  /**
   ...
   */
  static async getTemplates(directory, enforceOptions, type) {
  ...
  static async getConfig(directory) {
    ...
    try {
      ...
      return Object.assign(defaults, await resolve(require(`${directory}/sweeney.js`)));
    } catch(ex) {
      try {
        await stat(`${directory}/.sweeneyrc`);
        return Object.assign(defaults, await resolve(require(`${directory}/.sweeneyrc`)));
      } catch(ex) {
        return defaults;
      }
    }
