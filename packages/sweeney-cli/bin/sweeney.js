...
const Site = require('@sweeney/core/lib/site');
const Serve = require('@sweeney/core/lib/serve/serve');

const { time, getConfigInDirectory, copyDirectory, renderSubDepends, ensureDirectoryExists } = require('@sweeney/core/lib/util');
...
(async function () {
  try {
    ...
    let config = await getConfigInDirectory(program);
    ...
    if (program.watch) {
      ...
      fs.watch(site.source, {
        ...
        if (file.indexOf('.sweeney') > -1) {
          ...
          site.config = await getConfigInDirectory(program);
          await site.build();
        }
